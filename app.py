...
RCA_STANDARDS = {
    ...
    "CCTV System": ["BNC/RJ45 Connector Corrosion", "Power Supply Unit Fault", "IP Conflict", "Lens Fogging", "Lightning Damage"],
...
def init_connection():
    ...
    try:
        ...
        # Pointing specifically to 'Sheet1' for inventory
        try:
            inv = sh.worksheet("Sheet1")
        except:
            st.error("Error: 'Sheet1' not found. Please rename your inventory tab to 'Sheet1'.")
...
st.markdown("""
    ...
    <div class="header-style">
        ...
        <p style="margin:0;">Electromechanical Master Database (Source: Sheet1)</p>
    </div>
...
if menu == "📊 Dashboard":
    if not df_inv.empty:
        ...
        m1.metric("💰 Total Asset Value", f"{val_sum:,.2f} Br")
        m2.metric("📦 Equipment Count", int(qty_sum))
        ...
        with r:
            ...
            fig_h = px.bar(h_df.sort_values('Health %'), x='Health %', y=c_col, orientation='h', text='Health %', color='Health %', color_continuous_scale='RdYlGn', height=350, template="plotly_white")
            fig_h.update_traces(texttemplate='%{text}%', textposition='outside')
            st.plotly_chart(fig_h, use_container_width=True)

# --- 7. DYNAMIC REGISTRATION (Writing to Sheet1) ---
elif menu == "📝 Register New Equipment":
    st.subheader("📝 New Asset Registration (Sheet1)")
    c1, c2 = st.columns(2)
    ...
    with st.form("reg_form", clear_on_submit=True):
        ...
        if st.form_submit_button("✅ Register to Sheet1"):
            if reg_code:
                ...
                new_asset = [reg_cat, reg_sub, reg_code, reg_unit, reg_qty, reg_qty, reg_cost, total_val, reg_life, 0, 0]
                inv_ws.append_row(new_asset)
                st.success(f"Asset {reg_code} successfully added to Sheet1!"); st.rerun()
...
# --- 8. DYNAMIC MAINTENANCE LOG ---
    ...
    with st.form("maint_form", clear_on_submit=True):
        cause_opts = RCA_STANDARDS.get(m_cat, []) + RCA_STANDARDS["General"]
        m_cause = st.selectbox("Root Cause", cause_opts)
        ...
        if st.form_submit_button("Submit Technical Report"):
            if m_code and m_tech:
                ...
                st.success("Log submitted to Maintenance_Log!"); st.rerun()
    ...
    if not df_inv.empty:
        ...
        if st.button("💾 Sync Database"):
            ...
            st.success("Sheet1 synced!"); st.rerun()
