...
with st.sidebar:
    ...
    st.divider()
    st.info("System: AAE-EMS v9.5 (Fixed)")

# --- 4. DATA HANDLING (FIXED KEYERROR) ---
sh = init_connection()
...
def get_safe_data(worksheet):
    ...
    if not data:
        return pd.DataFrame()
    # Header cleaning: Remove spaces and ensure standard naming
    headers = [str(h).strip() for h in data[0]]
    df = pd.DataFrame(data[1:], columns=headers) if len(data) > 1 else pd.DataFrame(columns=headers)
    
    # CRITICAL FIX: Ensure all required columns exist to prevent KeyErrors
    required_cols = ['Quantity', 'Unit Cost', 'Total Value', 'Expected Life', 'Current Age', 'Functional Qty', 'Non-Functional Qty', 'Category', 'Asset Name']
    for col in required_cols:
        if col not in df.columns:
            df[col] = 0 # Create the column if it's missing in the Google Sheet
            
    # Convert to numeric
    numeric_cols = ['Quantity', 'Unit Cost', 'Total Value', 'Expected Life', 'Current Age', 'Functional Qty', 'Non-Functional Qty']
...
if menu == "📊 Dashboard":
    if not df_inv.empty:
        ...
        aging_count = len(df_inv[df_inv['Current Age'] >= df_inv['Expected Life']]) if 'Current Age' in df_inv.columns else 0
        ...
        with c1:
            ...
            cat_summary['Health %'] = (cat_summary['Functional Qty'] / (cat_summary['Quantity'].replace(0, 1)) * 100).round(1)
            fig = px.bar(cat_summary.sort_values('Health %'), x='Health %', y='Category', orientation='h', range_x=[0, 100], text='Health %')
...
# MAINTENANCE LOG
    ...
    if not df_inv.empty:
        ...
        with st.form("m_form", clear_on_submit=True):
            ...
            cost = c2.number_input("Cost", min_value=0.0)
            if st.form_submit_button("💾 Save"):
                maint_ws.append_row([m_cat, target, str(datetime.date.today()), qty, "Nos", "", "Maintenance", "Update", cost])
